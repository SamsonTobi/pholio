...
- [x] 8.5 Implement `/api/mcp` (MCP Streamable HTTP-compatible JSON-RPC): Bearer auth, per-tool scope checks (`showcase:write`/`stats:read`/`leaderboard:read`), per-key 429 rate limit, route to existing services (`projects.update`, `showcases.publish`, `github-sync.reparse`, `mockups.upload(base64)`, `telemetry.getStats`, `leaderboard.get`). Thin wrapper only. DECISION (recorded): transport is hand-rolled JSON-RPC, not `@modelcontextprotocol/sdk` server transport — the dep stays declared for future zodToJsonSchema adoption, tracked as follow-up. Length caps in `mcpSchemas`; batch cap 25; `get_stats` returns `demo:true` offline / honest empty live; `group_slug` required with `-32602`.
